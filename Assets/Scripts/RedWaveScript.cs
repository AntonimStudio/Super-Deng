...
public class RedWaveScript : MonoBehaviour
{
        ...
        if (isTurnOn)
        {
            if (isRandomSpawnTime)
            {
                for (int i = 0; i < colvo; i++)
                {
                    ...
                    StartCoroutine(SetRedWave(faces[randomIndex], materialRed, new Vector3(1f, 1f, scaleChange), colorChangeDuration, scaleChangeDuration));
                }
            }
        }
    }

    private IEnumerator SetRedWave(GameObject face, Material targetMaterial, Vector3 targetScale, float colorDuration, float scaleDuration)
    {
        ...
        yield return StartCoroutine(ChangeScale(face, new Vector3(1f, 1f, scaleChange), new Vector3(0f, positionChange, 0f), scaleDuration, true));
        SetNextStep(face.GetComponent<FaceScript>());
        yield return new WaitForSeconds(0.2f);
       
        yield return StartCoroutine(ChangeScale(face, new Vector3(1f, 1f, 1f), new Vector3(0f, 0f, 0f), scaleDuration, false));
    ...
    private void SetNextStep(FaceScript facescript)
    {
        ...
        if (facescript.pathObjectCount != 0) StartCoroutine(SetRedWave(objectWithMinPathCounter.gameObject, materialRed, new Vector3(1f, 1f, scaleChange), colorChangeDuration, scaleChangeDuration));
    }
}
